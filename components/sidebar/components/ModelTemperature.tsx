...
const ModelTemperature: React.FC<ModelTemperatureProps> = ({
    ...
    return (
        <div>
            <label className="block font-medium leading-6 text-xs sm:text text-blue-400">
                Model Temperature
            </label>
            <div className="mt-2">
                <input type="number" min={0} max={1} value={modelTemperature} onChange={handleTemperatureChange} step={0.1}
                    ...
                    name="temperature" id="temperature" placeholder="0.0 - 1.0" />
            </div>
        </div>
    );
};
